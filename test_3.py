    ...
    def step(self, action):
        ...
        if done:
            ...
            try:
                ...
                # 将总 reward 定义为准确率，或者乘以一个系数来调整尺度
                reward = total_accuracy * 10  # 或者根据需要调整系数
            except Exception as e:
                print(f"Error during accuracy calculation or tree copying: {e}")
                reward = -0.1  # 若出现错误，则惩罚
        ...
        # 返回观测状态、延迟的总 reward（仅在 done 时生效）、done 标志等
...
if __name__ == "__main__":
    ...
    print(tree_rules)
    with open(f"pic/{DATA_NAME}_sklearn_tree.txt", "w") as f:
        f.write("cart\n")
        f.write(tree_rules)
        f.write(f"Accuracy: {cart_accuracy:.4f}\n")

        f.write("final\n")
        f.write(str(final_tree.nodes))
